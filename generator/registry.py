...
def build_augmented(visitor):
    ...
    # `lore_link_entry_event_data_t`). `events_types.ji` emits a blittable
    # `_<Class>` struct and a public `<Class>` data class for every entry in
    # `visitor.events`, so registering all of them means a newly-nested event
    # type needs no registry edit.
    for struct_name in visitor.events:
        if struct_name.endswith("_array_t"):
            continue
        wrapper_class = util.pascal_case(struct_name.removesuffix("_t"))
        cs_map.setdefault(struct_name, f"_{wrapper_class}")
